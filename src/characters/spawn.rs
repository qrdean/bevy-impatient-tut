...
pub struct CharactersListResource {
    pub handle: Handle<CharacterList>,
}
...
pub fn spawn_player(
    ...
    let character_list_handle: Handle<CharacterList> =
...
pub fn initialize_player_character(
    ...
    character_lists: Res<Assets<CharacterList>>,
    ...
    for entity in query.iter_mut() {
        let Some(character_list) = character_lists.get(&characters_list_res.handle) else {
            continue;
        };

        if character_index.index >= character_list.characters.len() {
            continue;
        }

        let character_entry = &character_list.characters[character_index.index];
...
pub fn switch_character(
    ...
    let Some(characters_list) = characters_lists.get(&character_list_res.handle) else {
        return;
    };
